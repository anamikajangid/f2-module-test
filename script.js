...
function displayData(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    str =
      ...
      <img src="edit 1.png" id="edit-id${arr[i].ID}" alt="edit">
      </button>
      <button id="trash-button" class="btn" onclick="deleteData(event)">
      <img src="trash-2 1.png" id="trash-id${arr[i].ID}" alt="trash">
      </button>
      </div>
      </td>
  </tr>
...
document
  .getElementById("search-student-id")
  .addEventListener("keyup", (Event) => {
    ...
    let filteredData = studentData.filter((item) => {
      ...
      return (
        ...
        sEmail.includes(searchTerm) ||
        sDegree.includes(searchTerm)
      );
    });
